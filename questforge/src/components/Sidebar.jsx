  ...
  return (
    <aside className="sidebar">
      <div className="parch-card">
        ...
        <div className="xp-lbl">✦ Experience</div>
        <div className="xp-track"><div className="xp-fill" style={{ width: pct + '%' }} /></div>
        <div className="xp-num">{xp} / {needed} XP</div>
        <div className="stat-row">
          <div className="stat-cell"><div className="stat-val">{profile?.total_done || 0}</div><div className="stat-lbl">Quests</div></div>
          <div className="stat-cell"><div className="stat-val">{profile?.gold || 0}</div><div className="stat-lbl">Gold</div></div>
          <div className="stat-cell"><div className="stat-val">{profile?.current_streak || 0}</div><div className="stat-lbl">Streak</div></div>
        </div>
      </div>

      <div className="torch-card">
        <div className="widget-title">✦ Daily Flame ✦</div>
        <span className="big-flame">🔥</span>
        <div className="streak-num">{profile?.current_streak || 0}</div>
        <div className="streak-sub">days of glory</div>
      </div>

      <div className="timer-card">
        <div className="widget-title">⏳ Hourglass Focus</div>
        <div className="timer-num">{timeStr}</div>
        <div className="timer-sub">{timerOn ? `⏳ Focus! ${timerSec}s remaining` : '+15 XP upon completion'}</div>
        <button className="btn-timer" onClick={onTimerToggle}>
          {timerOn ? '⏹ Abandon Quest' : '⏳ Begin Thy Quest'}
        </button>
      </div>
